...
4. Verify each component against its baseline via image diff on the matching surface via the control skill. A nonzero diff is a fail. Investigate the pixel delta. Iterate per component until the diff is zero; use bounded waits or authorized native scheduling per RUNTIME.md.
