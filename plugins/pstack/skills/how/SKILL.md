...
After the explanation is complete, spawn one architectural critic per model in the `how critics` panel resolved per CODEX.md, all in a single message.

For each critic:
- `model`: one choice from the resolved `how critics` panel; pass model and reasoning separately per CODEX.md. Bundled efforts are minimum reasoning levels. Escalate supported effort when the architecture warrants deeper analysis, unless the user set an explicit effort limit.
- Scope: read only. Use the tools available to the child; do not change its sandbox to obtain MCP access.
