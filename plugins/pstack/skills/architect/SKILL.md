...
Use the `architect runners` panel resolved per RUNTIME.md, passing that panel to arena instead of its default runners.
...
Present the synthesized design before implementation. If implementation is already authorized in the session, continue within that scope. Otherwise stop with the design for review. An architecture-only request does not authorize implementation.
