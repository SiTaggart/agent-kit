...
> Codex port of Lauren Tan's upstream PStack, pinned to `93b00b89ef425a9c1bac0d0b317dfc49c930ac99` (v0.14.8). The original guide and author voice follow, with direct host adaptations described in [CODEX.md](./CODEX.md). This package stops at merge-ready; the user merges.
...
For this unpublished checkout, run from the Agent Kit repository root:
...
```
$poteto-mode i'm going to bed. verify the stack even if ci flakes. i want everything merge-ready by
morning.
```
...
| [shipping](./skills/poteto-mode/playbooks/shipping.md) | independently verify a green stack, then hand the contiguous verified run to the user for bottom-up merging. |
...
| [autopilot-full](./skills/poteto-mode/playbooks/autopilot-full.md) | run independent prs to merge-ready with one owner per pr and root verification of each merge-ready head. |
...
overnight run:     $poteto-mode i'm going to bed. verify the stack even if ci flakes. i want
                   everything merge-ready by morning.
