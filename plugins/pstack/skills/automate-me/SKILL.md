...
This skill orchestrates three others: an inline mining pass (see step 1), the selected adapter’s skill-authoring workflow, and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `<project-skills>/**/*-mode/SKILL.md` and `<personal-skills>/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`<project-skills>/<handle>/`), not only at the top level. If one exists, confirm intent with the available user-input tool (unless they already said "update my skill" or similar):
...
Mining misses intent that hasn't come up yet. Use the available user-input tool (structured multi-choice) rather than asking the user to type from scratch.

Shape: one or two questions using the available tool schema. Use multi-select only when the live schema supports it; otherwise ask for applicable categories in free text. Keep single-choice options within the tool's supported count. Start broad ("Which areas matter most?"), then follow up on selected areas with specific options. After the structured rounds, one free-form chat question catches anything the options missed.
...
Follow the selected adapter’s skill-authoring workflow. Placement:
...
- Use the selected adapter’s explicit-invocation policy by default. Change it only when the user explicitly requests automatic activation.
...
Work in a worktree off main. Commit and open a PR so the user can review it when publication is authorized in the session. Otherwise hand back the verified local skill. Don't push to main directly.
