...
1. Locate the prior trail. A project-scoped host session history or transcript discovered per RUNTIME.md, a host session link, or a pushed branch. Validate project/task metadata before reading and never glob unrelated chats. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
