...
This skill orchestrates three others: an inline mining pass (see step 1), the installed `skill-creator` skill (authoring), and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.agents/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.agents/skills/<handle>/`), not only at the top level. If one exists, confirm intent with the available user-input tool (unless they already said "update my skill" or similar):
...
Mining misses intent that hasn't come up yet. Use the available Codex user-input tool (structured multi-choice) rather than asking the user to type from scratch.

Shape: one or two questions using the available Codex tool schema. It has no `allow_multiple` field; for category choices, ask for all applicable categories in free text. Keep single-choice options within the tool's supported count. Start broad ("Which areas matter most?"), then follow up on selected areas with specific options. After the structured rounds, one free-form chat question catches anything the options missed.
...
- Set `policy.allow_implicit_invocation: false` in `agents/openai.yaml` by default. Opt out only if the user explicitly wants their mode to apply on every turn.
