...
At the end of the run, before handing back, check the log told the truth. Use the Codex history workflow in CODEX.md. Identify this project and the exact task before reading messages. If the transcript is unavailable, use a labeled session digest and report the missing evidence. Walk the log against what actually happened:
...
Before handing back, you must spawn a subagent using the `arena cross-judge pool` selection rules from CODEX.md. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
