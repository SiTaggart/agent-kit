...
When explicitly invoked, keep this mode active for the current conversation until
the user opts out. Apply it when a new task matches a playbook or needs rigor;
stay out of casual turns. Installation alone does not activate it.
...
- Before commit → the bundled `deslop` skill (ported from `cursor-team-kit`) (`/deslop`).
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → the matching control skill. The bundled `control-cli` covers CLIs and TUIs; `browser-use` covers browser and web UIs. For native desktop or Electron surfaces the browser cannot attach to, use the host's native computer-use surface per `browser-use`. For bug fixes, reproduce first on the same surface yourself. Hand to the user only under the narrow Bug fix step 1 exception.
- Any PR-status request → the **Babysit** playbook (`playbooks/babysit.md`). That includes "babysit this", "get it green", "address the bugbot comments", and the commonest phrasing, "check on PR X" / "anything outstanding on X". Never triggered by merely opening a PR. Declare its mode before polling. The playbook's step 1 owns the request-to-mode mapping. Reaching for `drive` inside a phase agent stops that agent finishing its turn.
- Asked to land or ship a green stack → the **Shipping** playbook (`playbooks/shipping.md`). Green is not safe. Nothing is declared merge-ready before an independent per-PR verdict, and only the contiguous verified run from the root is handed back for the user to merge.
- Bugbot or the agentic security review commented → skeptical posture. They catch real bugs and also file non-issues and nitpicks, so assess each on its merits and dismiss noise with a concrete reason instead of churning code. Triage fix / dismiss / ask per `references/bugbot-triage.md`.
- Broken skill mid-task → report the failure and use a bounded fallback. Repair the skill source only within authorized scope; never patch the installed cache.
...
- **Shipping.** The half after Babysit. Independently verifying a green stack, then handing back the contiguous verified run bottom-up for the user to merge through `gh` or Origin when its CLI is available. `playbooks/shipping.md`.
