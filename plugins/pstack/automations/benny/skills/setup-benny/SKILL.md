...
Do this before asking for Benny configuration and before live automation provisioning. Read the runtime preflight first.
...
Do not count a skill loaded from the current session or a user-scoped plugin alone. The check must show that a fresh automation agent in the target repository receives the installed PStack version and all required skills.

If the required project-scoped delivery guarantee is unavailable, the fresh automation runtime cannot load PStack, or any shared dependency does not resolve, stop and explain the failure.
...
Tell the user that `.agents/automations/benny/` and any referenced secret-free configuration must be committed before either automation is enabled. Do not commit them unless the user asks.
...
Prefer committed, secret-free files in the target repository when a fresh automation checkout must read them. Otherwise paraphrase the required values into the live prompt. Reference a repository file only after verifying that the file is committed in the repository where the automation runs.
...
For each automation, only after the runtime preflight proves native trigger support:
...
5. Discover the current native automation tool and read its schema.
6. Verify Slack channels, repository, and connected integrations using available tools.
7. Verify the copied pack and referenced configuration are committed in the repository where the automation will run.
8. Present the complete draft, verify existing authorization, and check readiness before enabling it.
9. Finish and read back this automation before starting the next one.

The current `automation_update` tool supports scheduled heartbeat and cron jobs, not Slack event triggers. Stop before creation; do not coerce either into this workflow.

Prepare this complete triage intent, filled from configuration:
...
After triage provisioning is supported and complete, prepare this complete repro and fix intent:
...
- Include pull request creation and the configured tracker, control-adapter, and feature-map requirements. Paraphrase mapped user paths and states unless you verify an eligible committed file in the same repository.
...
Reuse native tool checks for Slack, repository, integration, completeness, authentication, draft review, authorization, readiness, and readback. Do not bypass missing capabilities.

### Existing automations

Inspect the existing native automation before proposing an update. Preserve fields outside the requested change and do not create a duplicate. The current scheduler cannot represent the required Slack trigger; fail closed instead of changing the trigger to a schedule.
...
Use only supported native update capabilities after authorization and the runtime preflight. Read back the exact updated automation. Do not create replacements or duplicates.

### Creation boundary

Use the native automation tool only with arguments supported by its current schema. Do not call an undocumented backend, craft protocol links, create a trigger service, or substitute scheduled polling for Slack events. The current host cannot complete faithful provisioning.
