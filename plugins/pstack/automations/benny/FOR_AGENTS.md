...
if the automation runtime cannot load the installed plugin, the required project-scoped delivery guarantee is unavailable, or any shared dependency does not resolve, stop and explain what failed. do not add `.agents/automations/benny/skills/` to a plugin manifest or expect its files to appear in the slash-skill list.
...
for existing automations, validate the configuration, inspect the exact native automation and its trigger support, then use the copied setup checklist. Do not alter a scheduled automation into an unsupported Slack trigger or create duplicates.
