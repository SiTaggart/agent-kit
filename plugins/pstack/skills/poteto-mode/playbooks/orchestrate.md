...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, messaged, resumed, and drained only through the native Codex collaboration tools. Use spawn for new workers, send_message for active workers, followup_task for an idle worker that must resume, list_agents for status, and wait_agent for bounded waits. Do not create sidebar tasks as workers. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
...
- **Worker / verifier.** Use local Codex subagents. Each writer gets an exclusive worktree or branch; give parallel runtime lanes separate application data, output paths, and ports. Browser-use, control-cli, project-scoped history, simulators, IDE state, and local authentication share this machine. The native tool here has no durable cloud-worker equivalent; if VM isolation or remote-only resources are essential, record a blocked verification rather than claiming equivalence. Briefs inline what workers need or point at absolute plugin and repository paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Resolve workers through `swarm workers` and select a verifier from `arena cross-judge pool`, preferring a different available model per CODEX.md. OpenAI-only choices do not provide cross-provider independence.
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (use authorized native Codex scheduling with a long heartbeat fallback; otherwise bounded current-turn waits), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts; a dedicated verifier agent (on a different available model than the worker, resolved per CODEX.md; OpenAI-only choices do not provide cross-provider independence) is for units whose verification is expensive, judgment-laden, or high-blast-radius. A verifier agent whose entire product would be rerunning one command is ceremony, not verification.
