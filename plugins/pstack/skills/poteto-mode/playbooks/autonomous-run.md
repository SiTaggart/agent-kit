...
1. State the exit condition as a checkable predicate before the first iteration (tests green, repro fixed, all N PRs verified and merge-ready, pixel-diff zero). A vague goal stalls; a predicate lets you stop.
2. Pick the wake mechanism using Codex native scheduling for an authorized recurring run, or bounded waits during the current turn, per CODEX.md. An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
