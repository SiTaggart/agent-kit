...
**Merge boundary.** In this port, landing means integrating verified work into its review branch and handing back a merge-ready PR. It never means merging a PR. Observe user merges before advancing a dependent frontier. Keep merged and merge-ready as separate recorded states.
...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, messaged, resumed, and drained only through the native collaboration tools. Use the selected runtime adapter for spawning, messaging, status, bounded waits, and resuming an idle worker. If a required operation is unavailable, report it and leave a resume point. Do not create sidebar tasks as workers. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Local with a durable store, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (only when the current native tool permits nested delegation; never assume a fixed supported depth or a cloud environment field). Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, no more than the current native concurrency limit permits across the whole tree, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Use local native subagents. Each writer gets an exclusive worktree or branch; give parallel runtime lanes separate application data, output paths, and ports. Browser-use, control-cli, project-scoped history, simulators, IDE state, and local authentication share this machine. The native tool here has no durable cloud-worker equivalent; if VM isolation or remote-only resources are essential, record a blocked verification rather than claiming equivalence. Briefs inline what workers need or point at absolute plugin and repository paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Resolve workers through `swarm workers` and select a verifier from `arena cross-judge pool`, preferring a different available model per RUNTIME.md. Same-provider choices do not provide cross-provider independence.
...
Create `.ai/orchestrate/<project-slug>/` in the consuming repository. Every file has exactly one writer. Owners publish facts, readers aggregate at read time. Use `bun "<absolute-plugin-root>/skills/poteto-mode/scripts/orch/orch.ts" --store "<absolute-consuming-repo>/.ai/orchestrate/<project-slug>"` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, and an optional default only when a preference can safely default; permission never defaults) so a completion flood cannot wipe pending user-input state.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape. A 4KB scaffold around a two-line edit costs more to write and obey than the edit. Paste the standing orders verbatim into every spawn and every resume. Also pass the absolute store path so each worker can read the current register.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget within the native tree-wide concurrency limit and its worktree/runtime isolation assignments, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (use authorized native scheduling with a long heartbeat fallback; otherwise bounded current-turn waits), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack. Record the holder in the standing orders. Restacks use an exclusive local worktree and serialize machine-heavy work. If the program needs remote capacity to avoid overloading this host, record that missing capacity instead of pretending local workers are cloud-isolated.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts. A dedicated verifier agent (on a different available model than the worker, resolved per RUNTIME.md; Same-provider choices do not provide cross-provider independence) is for units whose verification is expensive, judgment-laden, or high-blast-radius. A verifier agent whose entire product would be rerunning one command is ceremony, not verification.
...
A unit is not done until its output is externalized the moment it lands, never batched to the end of the run. A worker pushes its branch, a verifier writes its ledger row, receipts land in the store. Work that exists only in a transient worker without a durable commit or receipt was never done.

#### Liveness and failure

- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the native agent's current status from the host status tool and any live tool handle from its latest result. Transcript mtime is not liveness. An observation timeout is not terminal; re-poll the same handle or inspect authoritative status before replacement.
...
- After a host restart: re-read the standing orders and `units.tsv`, recompute the frontier, and inventory native agent handles. Do not assume local workers survived or that a missing handle means their branch output was lost. Reconcile each worker through PR, branch, head SHA, and receipts; reattach monitoring to a confirmed live handle, resume an idle worker only when its assigned work must continue, or respawn a missing/terminal worker from its consolidated brief and current state. Respawn sub-coordinators only where the stored decomposition still needs them, drain, resume. Durable cloud execution across restart has no direct equivalent in this host. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.

#### Escalation

Reaches the human, batched into the status page rather than per item: irreversible actions without existing authorization (force-push to shared branches, deploys, deletions, closing someone else's PR), genuine product or preference calls no experiment settles, a standing order that contradicts observed reality, a program-level dead end that survived a replan. Park each as a `gates.md` entry before asking, and route work around it.
