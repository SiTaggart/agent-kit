...
set a Codex goal and continue until done. use a heartbeat if you need to check back later.
...
- A Codex goal keeps the active task working toward its finish condition. An explicitly requested heartbeat uses Codex's automation tool to check back later; it stays quiet while nothing actionable changes. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) uses native events and requested heartbeats instead of Cursor's `/loop`. If the host exposes neither continuation mechanism, the agent reports that limit rather than promising a wake-up.
...
Before the skill hands back its summary, it spawns a reviewer on a different available model (the bundled models share one provider) to read the trail and the transcript, and the reply ends with an Attention section listing what deserves your scrutiny. Read that section first, then the log rows it points at. You're auditing decisions, not re-reading the whole night.
...
[Autopilot-stack](../../skills/poteto-mode/playbooks/autopilot-stack.md) runs the same owner loop but ships nothing. You wake up to one linear base-branch stack with a verifier's verdict on every link, and you review and land it yourself. Pick it over Autopilot-full when the changes are coupled, or when you want to review one ordered stack:
