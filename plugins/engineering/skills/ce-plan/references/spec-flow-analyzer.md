...
*Analyzes specifications and feature descriptions for user flow completeness and gap identification. Dispatched by `ce-plan` when a spec, plan, or feature description needs flow analysis, edge case discovery, or requirements validation before planning proceeds.*
...
1. Use your platform's content-search tools to find code related to the feature area -- models, controllers, services, routes, existing tests
2. Use your platform's file-search tools to find related features that may share patterns or integrate with this one
